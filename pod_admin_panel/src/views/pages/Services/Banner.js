...
class Banner extends Component{
    ...
    try{
      ...
      if(getBannerImages.data.success===true){
       ...
       if(getBannerImages.data.data.length !==0){
         ...
         const fetchedImages = JSON.parse(getBannerImages.data.data[0].image)
         console.log('fetched array of Carousel-->',fetchedImages.image)
         if(fetchedImages.image !== undefined){
           this.setState({
             image: fetchedImages.image
           });
         }
        }
       }
  ...
  handleSubmit = async (event) => {
    ...
    if (Object.values(formValidity).every(Boolean)) {
      console.log("data-->", formValues, this.state.imagesArray);
